# Triangulations of the quaternionic projective plane and manifolds like the octonionic projective plane
...
Alongside with programs, the repository contains the data files with lists of simplices of the constructed triangulations:

+ Files **like_OP2_27_K1.dat** - **like_OP2_27_K4.dat** contain the lists of representatives of $G_{351}$-orbits of maximal simplices for the 4 combinatorial 16-manifolds like the octonionic projective plane with symmetry group $G_{351}$. These combinatorial manifolds were constructed in [G1]. Here $G_{351}$ is the subgroup of $S_{27}$ generated by the two permutations
```
...
**Remark** The programs **triple_flip_graph**, **triple_flip_graph_random**, **isomorphism_groups**, and **symm_group** are written so that they work for only 15-vertex combinatorial $8$-manifolds like the quaternionic projective plane. For the first two programs is the reason is that this is the only case when the triple flip graphs $\mathcal{G}_G$ are defined, non-trivial and accessible to a computer. (In dimension 16 the corresponding graphs are far beyond the capabilities of a computer, and in dimensions 2 and 4 they are trivial.) For the last two programs the only reason is just that we want to use the common library **triang.cpp / triang.hpp**, which was written specifically for the programs **triple_flip_graph** and **triple_flip_graph_random** and hence is applicable in the case of 15-vertex combinatorial $8$-manifolds like the quaternionic projective plane only.
...
+ A file **symmetry_group.dat** describing the symmetry group $G$ in the standard format.
+ A file **triang.dat** describing a pure $d$-dimensional simplicial complex $K$ in the standard format.

**Output data:**

+ A file **fvect.dat** containing the numbers $f_0(K),\\ldots,f_d(K)$, each number in a separate line.
